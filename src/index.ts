...
const parse = async (date: moment.Moment) => {
  ...
  const result = lines.map((line, index) => {
    try {
      ...
      //诊断类型
      const diagnosedParams = params.filter(
        (one) => one.indexOf("诊断") > -1
      )[0];
      info.diagnosed =
...
(async () => {
  for (let index = 22; index < 31; index++) {
    const startDate = moment(`2021-12-${index}`);
    const result = await parse(startDate);
    console.log(result.filter((one) => one.reason === "密切接触"));
  }
})();
